...
+(void)likeMomentWithPriseid:(NSString *)enterpriseId successBlock:(void(^) (NSString *successMsg))success failBlock:(void(^) (NSError *error))failure{
    NSString *urlStr =  [WXApiManager getRequestUrl:@"enterlike/enterpriselikeadd"];//后面一个单词可能有错
    NSDictionary *params = @{@"enterprisezid":enterpriseId,@"enterpriseliketid": WXAccountTool.getUserID,@"enterpricelikename": userName};//后两位参数就是userid和username
    [WXNetWorkTool requestWithType:WXHttpRequestTypePost urlString:urlStr parameters:params successBlock:^(id  _Nonnull responseBody) {
        NSString *code = [NSString stringWithFormat:@"%@",responseBody[@"code"]];
        if ([code isEqualToString:@"200"]){
            //成功
...
+(void)commentWithContent:(NSString *)content priseid:(NSString *)priseid successBlock:(void(^) (NSString *successMsg))success failBlock:(void(^) (NSError *error))failure{
    ...
    NSDictionary *params = @{@"commentszid": priseid,@"commentstguset": WXAccountTool.getUserID,@"commentscontext": content,@"commentstgusetname": userName};//2和4是userid和name
    [WXNetWorkTool requestWithType:WXHttpRequestTypePost urlString:urlStr parameters:params successBlock:^(id  _Nonnull responseBody) {
        NSString *code = [NSString stringWithFormat:@"%@",responseBody[@"code"]];
        if ([code isEqualToString:@"200"]){
            //成功
...
+(void)getMomentMessageListSuccessBlock:(void(^) (MomentMessageList *model))success failBlock:(void(^) (NSError *error))failure{
    ...
    [WXNetWorkTool requestWithType:WXHttpRequestTypePost urlString:urlStr parameters:params successBlock:^(id  _Nonnull responseBody) {
        ...
        if ([code isEqualToString:@"200"]){
            //成功
            MomentMessageList *model = [MomentMessageList yy_modelWithJSON:responseBody[@"data"]];
            success(model);
        }else{
            [MBProgressHUD showError: responseBody[@"msg"]];
        }
...
+(void)publicMomentsMessage:(NSString *)message files:(NSString *)files successBlock:(void(^) (NSString *successMsg))success failBlock:(void(^) (NSError *error))failure{
    NSString *urlStr =  [WXApiManager getRequestUrl:@"enterprisez/tupship"];
    NSDictionary *params = @{@"Tgusetaccount": WXAccountTool.getUserPhone,@"enterprisezcontent": message,@"file": files,@"tgusetid":WXAccountTool.getUserID};
    [WXNetWorkTool requestWithType:WXHttpRequestTypePost urlString:urlStr parameters:params successBlock:^(id  _Nonnull responseBody) {
        NSString *code = [NSString stringWithFormat:@"%@",responseBody[@"code"]];
        if ([code isEqualToString:@"200"]){
            //成功
