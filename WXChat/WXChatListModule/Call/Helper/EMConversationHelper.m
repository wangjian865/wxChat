...
+ (NSArray<EMConversationModel *> *)modelsFromEMConversations:(NSArray<EMConversation *> *)aConversations
{
    ...
    for (int i = 0; i < [aConversations count]; i++) {
        EMConversation *conversation = (EMConversation *)aConversations[i];
...
+ (EMConversationModel *)modelFromContactMessage:(NSDictionary *)ContactMessageDic{
    EMConversation *conversation = [[EMClient sharedClient].chatManager getConversation:ContactMessageDic[@"worknumber"] type:EMConversationTypeChat createIfNotExist:YES];
    EMConversationModel *model = [[EMConversationModel alloc] initWithEMModel:conversation];
    model.name = ContactMessageDic[@"worknumber"];
    model.nickName = ContactMessageDic[@"nickname"];
    model.imageUrlme = ContactMessageDic[@"imageurlme"];
    model.imageUrlfriend = ContactMessageDic[@"imageurlfriend"];
    return model;
}
